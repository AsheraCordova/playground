...
export abstract class RadioGroupImpl<T> extends ViewGroupImpl<T>{
	//start - body
	static initialize() {
	...
	@Type(() => CommandAttr)
	@Expose({ name: "checkedButton" })
	checkedButton!:CommandAttr<string>| undefined;
	@Type(() => CommandAttr)
...
this.gravity.setTransformer('gravity');		return this.thisPointer;
	...
	public tryGetCheckedButton() : T {
		this.resetIfRequired();
		if (this.checkedButton == null || this.checkedButton == undefined) {
			this.checkedButton = new CommandAttr<string>()
		}
		
		this.checkedButton.setGetter(true);
		this.orderGet++;
		this.checkedButton.setOrderGet(this.orderGet);
		return this.thisPointer;
	}
	
	public getCheckedButton() : string {
		if (this.checkedButton == null || this.checkedButton == undefined) {
			this.checkedButton = new CommandAttr<string>();
		}
		return this.checkedButton.getCommandReturnValue();
	}
	public setCheckedButton(value : string) : T {
		this.resetIfRequired();
		if (this.checkedButton == null || this.checkedButton == undefined) {
			this.checkedButton = new CommandAttr<string>();
		}
		
		this.checkedButton.setSetter(true);
		this.checkedButton.setValue(value);
		this.orderSet++;
		this.checkedButton.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
...
//start - staticinit
export abstract class RadioGroupImpl_LayoutParams<T> extends ViewGroupImpl_LayoutParams<T> {
	@Type(() => CommandAttr)
...
export class RadioGroup_LayoutParams extends RadioGroupImpl_LayoutParams<RadioGroup_LayoutParams> implements ILayoutParam {
    getThisPointer(): RadioGroup_LayoutParams {
        return this;
    }
...
export class RadioGroup extends RadioGroupImpl<RadioGroup> implements IWidget{
    getThisPointer(): RadioGroup {
        return this;
    }
